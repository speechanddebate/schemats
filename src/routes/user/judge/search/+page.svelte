<script lang="ts">
	...
	const requestLink = async (row: UnlinkedJudge) => {
		...
		if (res.status === 204) {
			toast.success({ message: 'Link request submitted. Awaiting coach/tournament approval.' });
			await linkRequestsQuery.refetch();
		} else {
			toast.error({ message: 'Failed to submit link request. Please try again later.' });
		}
	};
